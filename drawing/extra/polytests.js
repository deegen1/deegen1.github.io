...
function ovalstresstest() {
	...
	var imgwidth=257;
	var imgheight=257;
	var img=new Draw.Image(imgwidth,imgheight);
	var draw=new Draw();
	draw.setimage(img);
	...
	for (var test=0;test<tests;test++) {
		draw.setcolor(rnd.getu32());
		var xrad=Math.pow(2,rnd.getf64()*10);
		var yrad=Math.pow(2,rnd.getf64()*10);
...
function imagestresstest() {
	...
	var draw=new Draw();
	var images=10;
	var imgarr=new Array(images);
	draw.setcolor(0,0,0,255);
	var amask=draw.rgba32[0],nmask=(~amask)>>>0;
	for (var i=0;i<images;i++) {
		var w=i>0?Math.floor(Math.pow(2,i*1.10)+1):0;
		var h=i>0?Math.floor(Math.pow(2,i*1.13)+1):0;
		var img=new Draw.Image(w,h);
		imgarr[i]=img;
		var data32=img.data32,datalen=data32.length,col;
	...
	var time=performance.now();
	for (var test=0;test<tests;test++) {
		var dstimg=imgarr[rnd.modu32(images)];
		var srcimg=imgarr[rnd.modu32(images)];
		draw.setimage(dstimg);
		draw.setcolor(rnd.getu32());
		var w=rnd.modu32(srcimg.width*2);
		var h=rnd.modu32(srcimg.height*2);
		var x=(rnd.getf64()*3-1)*dstimg.width;
		var y=(rnd.getf64()*3-1)*dstimg.height;
