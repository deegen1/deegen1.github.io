...
rm uint_test ; g++ -fsanitize=address -fsanitize=undefined uint_test.cpp -o\
uint_test -lm ; ./uint_test

rm uint_test ; g++ -O3 uint_test.cpp -o uint_test -lm ; ./uint_test
...
void SicoTestWrite(u64* str,u64 strlen,u64 stridx,u64* retidx,u64 num,u64 base,u64 mod) {
...
void UIntWriteStringTest(void) {
	printf("Testing uint.write\n");
	const char* header=R"(
		main:
			0 ? uint.write .arg0 .arg1 .arg2 .arg3 .arg4 .arg5
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: .str
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.write",header);
	SicoState* st=SicoCreate();
	for (u64 mod=630;mod<1040;mod++) {
		st->mod=mod;
		...
		for (u64 p0=0;p0<params;p0++) {
			...
			u64 calcidx=0;
			SicoTestWrite(calcstrptr,strlen,stridx,&calcidx,num,base,mod);
			instlimit[1]=80.0+9.0*((f64)(calcidx-stridx));
...
u64 SicoTestWrite36(u64* str,u64 strlen,u64 stridx,u64 num,u64 base,u64 padding,u64 padchar) {
	#define addchar(c) if (stridx<strlen) {str[stridx]=c;} stridx++;
	if (base>36) {
		return stridx;
	}
...
void UIntReadStringTest(void) {
	printf("Testing uint.read\n");
	const char* header=R"(
		main:
			0 ? uint.read .arg0 .arg1 .arg2 .arg3 .arg4 .arg5
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: .str
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.read",header);
	SicoState* st=SicoCreate();
	for (u64 mod=550;mod<900;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
		...
		f64 instlimit[38]={0};
		instlimit[0]=80.0;
		instlimit[1]=94.0+19.0*((f64)mod);
		for (u32 base=2;base<37;base++) {
			f64 digits=ceil(log(mod)/log(base));
			instlimit[base]=112+(17.0+2.0*base)*digits;
		}
		instlimit[37]=80.0;
		for (u64 p0=0;p0<params;p0++) {
			// Calculate the expected return value.
			u64 p=param[p0];
			u64 num=p%mod,base=(p/mod)%38,rand=p/(mod*38);
			// Randomize the argument order.
			u64 start=(SicoRandu64()%startrand)+main;
			SicoRandShuffle(argarr,args,1);
			// See if duplicate argument addresses will cause errors.
			...
			u64* calcstrptr=strptr?calcstr+(strptr-str0):0;
			u64 calcidx=SicoTestWrite36(calcstrptr,strlen,stridx,num,base,padding,padchar);
			if (rand) {
			...
			instlimit[1]=94.0+19.0*((f64)(calcidx-stridx));
			// Setup the function and abort instructions.
			SicoSetMem(st,start+0,0);
			SicoSetMem(st,start+1,start+1);
			SicoSetMem(st,start+2,func);
...
void UIntCmpTest(void) {
	printf("Testing uint.cmp\n");
	const char* header=R"(
		main:
			0 ? uint.cmp .arg0 .arg1 .arg2 .arg3 .arg4
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0-1 0 ?-2
			.arg1: 0-1 0 ?-2
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.cmp",header);
	SicoState* st=SicoCreate();
	for (u64 mod=150;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
		...
		for (u64 p0=0;p0<params;p0++) {
			...
			if (a==b || argarr[0]==argarr[1]) {calcret=argarr[3];}
			else if (a>b) {calcret=argarr[4];}
			// Setup the function and abort instructions.
			SicoSetMem(st,start+0,0);
			SicoSetMem(st,start+1,start+1);
			SicoSetMem(st,start+2,func);
...
void UIntMinTest(void) {
	printf("Testing uint.min\n");
	const char* header=R"(
		main:
			0 ? uint.min .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.min",header);
	SicoState* st=SicoCreate();
	for (u64 mod=150;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
...
void UIntMaxTest(void) {
	printf("Testing uint.max\n");
	const char* header=R"(
		main:
			0 ? uint.max .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.max",header);
	SicoState* st=SicoCreate();
	for (u64 mod=150;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
...
void UIntSetTest(void) {
	printf("Testing uint.set\n");
	const char* header=R"(
		main:
			0 ? uint.set .arg0 .arg1
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.set",header);
	SicoState* st=SicoCreate();
	for (u64 mod=110;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
...
void UIntNegTest(void) {
	printf("Testing uint.neg\n");
	const char* header=R"(
		main:
			0 ? uint.neg .arg0 .arg1
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.neg",header);
	SicoState* st=SicoCreate();
	for (u64 mod=110;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		...
		u64* argval=(u64*)malloc(args*sizeof(u64));
		u64 maxinst=0;
		const u64 instlimit=25;
		for (u64 p0=0;p0<params;p0++) {
			// Calculate the expected return value.
			u64 p=param[p0];
			u64 a=p%mod;
			u64 calcret=SicoNeg(a,mod);
			// Randomize the argument order.
			u64 start=(SicoRandu64()%startrand)+main;
			SicoRandShuffle(argarr,args,1);
			// See if duplicate argument addresses will cause errors.
...
void UIntAddTest(void) {
	printf("Testing uint.add\n");
	const char* header=R"(
		main:
			0 ? uint.add .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.add",header);
	SicoState* st=SicoCreate();
	for (u64 mod=130;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
...
void UIntSubTest(void) {
	printf("Testing uint.sub\n");
	const char* header=R"(
		main:
			0 ? uint.sub .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.sub",header);
	SicoState* st=SicoCreate();
	for (u64 mod=130;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		...
		u64* argval=(u64*)malloc(args*sizeof(u64));
		u64 maxinst=0;
		const u64 instlimit=31;
		for (u64 p0=0;p0<params;p0++) {
			// Calculate the expected return value.
			u64 p=param[p0];
			u64 a=p%mod,b=(p/mod)%mod;
			u64 calcret=SicoSub(a,b,mod);
			// Randomize the argument order.
			u64 start=(SicoRandu64()%startrand)+main;
			SicoRandShuffle(argarr,args,1);
			// See if duplicate argument addresses will cause errors.
...
void UIntMulTest(void) {
	printf("Testing uint.mul\n");
	const char* header=R"(
		main:
			0 ? uint.mul .arg0 .arg1 .arg2 .arg3
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.mul",header);
	SicoState* st=SicoCreate();
	for (u64 mod=430;mod<800;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
		...
		f64 instlimit[2]={48.0+7.21*bits,55.0+12.97*bits};
		for (u64 p0=0;p0<params;p0++) {
			// Calculate the expected return value.
			u64 p=param[p0];
			u64 a=p%mod,b=(p/mod)%mod;
			u64 high=p>=mod*mod;
			u64 calchigh=(a*b)/mod;
			u64 calclow=(a*b)%mod;
			// Randomize the argument order.
			u64 start=(SicoRandu64()%startrand)+main;
			SicoRandShuffle(argarr,args,1);
...
void UIntDivTest(void) {
	printf("Testing uint.div\n");
	const char* header=R"(
		main:
			0 ? uint.div .arg0 .arg1 .arg2 .arg3
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.div",header);
	SicoState* st=SicoCreate();
	for (u64 mod=360;mod<800;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
		...
		u64 arg0=SicoFindLabel(st,"main.arg0");
		u64 args=SicoFindLabel(st,"main.argn")-arg0;
		u64 divz=SicoFindLabel(st,"uint.div.divz");
		u64 startrand=arg0-main-args-5;
		// Randomize the parameter order.
		u64 params=mod*mod*2;
		u64* param=(u64*)malloc(params*sizeof(u64));
		...
		u64 maxinst[2]={0,0};
		f64 bits=log(mod)/log(2.0);
		f64 instlimit[2]={50.0+7.21*bits,60.0+8.65*bits};
		for (u64 p0=0;p0<params;p0++) {
			// Calculate the expected return value.
			u64 p=param[p0];
			u64 a=p%mod,b=(p/mod)%mod;
...
void UIntGCDTest(void) {
	printf("Testing uint.gcd\n");
	const char* header=R"(
		main:
			0 ? uint.gcd .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.gcd",header);
	SicoState* st=SicoCreate();
	for (u64 mod=225;mod<500;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
		...
		for (u64 p0=0;p0<params;p0++) {
			...
			SicoRandShuffle(argarr,args,1);
			// See if duplicate argument addresses will cause errors.
			argarr[0]=argarr[(SicoRandu32()&1)+1];
			for (u64 i=0;i<args;i++) {
				argarr[i]+=arg0;
				SicoSetMem(st,start+3+i,argarr[i]);
				argval[i]=SicoRandu64()%mod;
			...
			u32 error=0;
			if (((f64)st->insts)>instlimit) {
				printf("\ntoo many instructions: %" PRIu64 ", %f",st->insts,instlimit);
				error=1;
			}
			// Make sure the function cleaned up correctly.
			if (SicoGetMem(st,0)) {
			...
			// Check the return value.
			u64 retgcd=SicoGetMem(st,argarr[0]);
			if (calcgcd!=retgcd) {
				printf("\nreturn: %" PRIu64 "\nexpect: %" PRIu64,retgcd,calcgcd);
				error=1;
			}
			if (error) {
				printf("\nmod : %" PRIu64 "\n",mod);
		...
		printf("\rmod: %" PRIu64 ", instructions: %" PRIu64 ", %f",mod,maxinst,instlimit);
		fflush(0);
		free(argval);
		free(argarr);
		free(param);
...
void UIntShlTest(void) {
	printf("Testing uint.shl\n");
	const char* header=R"(
		main:
			0 ? uint.shl .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.shl",header);
	SicoState* st=SicoCreate();
	for (u64 bits=9;bits<12;bits++) {
		u64 mod=1<<bits;
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		...
		u64* argval=(u64*)malloc(args*sizeof(u64));
		u64 maxinst=0;
		u64 instlimit=56+16*((bits-1)/8);
		for (u64 p0=0;p0<params;p0++) {
			// Calculate the expected return value.
			u64 p=param[p0];
			u64 a=p%mod,s=p/mod;
			u64 calcshl=s<bits?(a<<s)%mod:0;
			// Randomize the argument order.
			u64 start=(SicoRandu64()%startrand)+main;
			SicoRandShuffle(argarr,args,1);
			// See if duplicate argument addresses will cause errors.
			...
			// Check the return value.
			u64 retshl=SicoGetMem(st,argarr[0]);
			if (calcshl!=retshl) {
				printf("\nreturn: %" PRIu64 "\nexpect: %" PRIu64,retshl,calcshl);
				error=1;
			}
			if (error) {
				printf("\nmod : %" PRIu64 "\n",mod);
...
void UIntShrTest(void) {
	printf("Testing uint.shr\n");
	const char* header=R"(
		main:
			0 ? uint.shr .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.shr",header);
	SicoState* st=SicoCreate();
	for (u64 bits=8;bits<12;bits++) {
		...
		u64* argval=(u64*)malloc(args*sizeof(u64));
		u64 maxinst=0;
		u64 instlimit=31+8*bits;
		for (u64 p0=0;p0<params;p0++) {
			// Calculate the expected return value.
			u64 p=param[p0];
			u64 a=p%mod,s=p/mod;
			u64 calcshr=s<bits?(a>>s):0;
			// Randomize the argument order.
			u64 start=(SicoRandu64()%startrand)+main;
			SicoRandShuffle(argarr,args,1);
			// See if duplicate argument addresses will cause errors.
...
void UIntNotTest(void) {
	printf("Testing uint.not\n");
	const char* header=R"(
		main:
			0 ? uint.not .arg0 .arg1
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.not",header);
	SicoState* st=SicoCreate();
	for (u64 mod=120;mod<300;mod++) {
		st->mod=mod;
		// Load the st source with the current mod.
		SicoParseAssembly(st,source);
		if (st->state!=SICO_RUNNING) {
		...
		printf("\rmod: %" PRIu64 ", instructions: %" PRIu64,mod,maxinst);
		fflush(0);
		free(argval);
		free(argarr);
		free(param);
...
void UIntAndTest(void) {
	printf("Testing uint.and\n");
	const char* header=R"(
		main:
			0 ? uint.and .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.and",header);
	SicoState* st=SicoCreate();
	for (u64 bits=8;bits<12;bits++) {
		u64 mod=1<<bits;
		st->mod=mod;
...
void UIntOrTest(void) {
	printf("Testing uint.or\n");
	const char* header=R"(
		main:
			0 ? uint.or .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.or",header);
	SicoState* st=SicoCreate();
	for (u64 bits=8;bits<12;bits++) {
		u64 mod=1<<bits;
		st->mod=mod;
...
void UIntXorTest(void) {
	printf("Testing uint.xor\n");
	const char* header=R"(
		main:
			0 ? uint.xor .arg0 .arg1 .arg2
			0-1 0 ?-2
			0 0 0 0 0
			.arg0: 0
			.arg1: 0
	...
	char* source=SicoLoadFunction("../library/uint.sico","uint.xor",header);
	SicoState* st=SicoCreate();
	for (u64 bits=8;bits<12;bits++) {
		u64 mod=1<<bits;
		st->mod=mod;
