/*------------------------------------------------------------------------------


rappel.js - v1.02

Copyright 2026 Alec Dee
2dee.net - akdee144@gmail.com


--------------------------------------------------------------------------------
Notes


Test demonstration for audio, graphics, physics, etc.

Player size = 2 units = 32 pixels

The hook should be tuned to slip when the player is at terminal velocity.


--------------------------------------------------------------------------------
TODO


Keep under 30kb, minus header.

Improve background music.


*/
/* npx eslint rappel.js -c ../../standards/eslint.js */


import {Env,Random,Vector,Matrix,Transform,Input,Draw,Audio,UI,Phy} from "./library.js";


// Material IDs
const NORM=0,WALL=1,BODY=2,ROPE=3,HOOK=4,EDGE=5;
const PART=6,LEAF=7,RAIN=8,CHAR=9;


export class Game {

	constructor(divid) {
		this.canvas=document.getElementById(divid);
		this.input=new Input(this.canvas);
		this.input.disablenav();
		this.mouse=new Vector(2);
		this.rnd=new Random();
		this.scale=16;
		this.drawrad=800/this.scale;
		this.maxspeed=50;
		this.chargerate=1;
		this.frames=0;
		this.framesum=0;
		this.framestr="0.0 ms";
		this.framemax=1/60;
		this.frameprev=0;
		this.initaudio();
		this.initgraphics();
		this.initphysics();
		this.initlevel();
		this.resize();
		let state=this;
		window.addEventListener("resize",()=>{state.resize();});
		function update(time) {
			if (state.update(time)) {
				requestAnimationFrame(update);
			}
		}
		requestAnimationFrame(update);
	}


	reset() {
		let world=this.world;
		for (let body of world.bodyiter()) {body.release();}
		world.gravity.set([0,6.66]);
		this.playerbody=null;
		this.charge=0;
		this.camera=new Transform({dim:2,scale:this.scale});
		this.camcen=new Vector(2);
		this.biomearr=[];
	}


	// ----------------------------------------
	// Audio


	initaudio() {
		let audio=new Audio(0,0,false);
		this.audio=audio;
		this.snddist2=30*30;
		// Create a high-tempo, dark, ambient background song.
		function eztri(freq) {
			return (new Audio.SFX(`#tri: TRI F ${freq} H 0.8 #out: ENV A 0.01 R 0.99 I #tri`)).tosound();
		}
		let bgsnd=new Audio.Sound();
		let notelib=[
			[0x1111,Audio.SFX.defsnd("hihat",1)],
			[0x4444,Audio.SFX.defsnd("kick",0.8)],
			[0x0101,eztri(73.416)],
			[0x0010,eztri(77.782)],
			[0x1000,eztri(82.407)]
		];
		for (let n=16*30-1;n>=0;n--) {
			let t=n/8,b=1<<(n&15);
			for (let note of notelib) {
				if (note[0]&b) {bgsnd.add(note[1],t);}
			}
		}
		bgsnd.resizetime(60);
		bgsnd.setloop(0);
		this.bgsnd=bgsnd;
		this.bgsndinst=this.bgsnd.play(0.3);
		// Create sound effects.
		this.spinsnd=(new Audio.SFX(`
			#spin: ENV A ${1/this.chargerate} S 100
			#freq: #spin 3 * 1 +
			#osc : SIN F #freq P 0.5 H 1 L 0
			#comb: 1 2 #osc * 2 ^ 1 + /
			#noi : NOISE H 0.5
			#bpf : BPF F #spin 400 * 200 + B 0.4 I #noi #comb *
			#out : ENV A 0.001 S 9.799 R 0.1 I #bpf
		`)).tosound();
		this.spinsnd.setloop(1/this.chargerate+0.1);
		this.spininst=null;
		this.windsnd=(new Audio.SFX(`
			#noi: NOISE H 0.8
			#out: LPF F 800 I #noi
		`)).tosound(5);
		this.windsnd.setloop(0);
		this.windinst=null;
		this.windvol=0;
		this.rainsnd=(new Audio.SFX(`
			#noi: NOISE
			#bp1: BPF F 150 B 2 I #noi
			#lp1: LPF F 500 I #bp1
			#saw: SAW F #lp1 80 * 40 +
			#cs1: SIN F #saw 0.25 -
			#mer: #lp1 #lp1 * 10 * #cs1 *
			#bp2: BPF F 500 I #mer 0.2 < -0.2 >
			#out: ENV A 0 S 0 R 2 I #bp2 1 *
		`)).tosound();
		this.leafsnd=(new Audio.SFX(`
			#wind1: SIN 0.25 H 1 L 0.5
			#wind2: #wind1 -0.4 * 0.7 +
			#noi: NOISE
			#hpf: HPF F 200 I #wind2 #noi > #wind2 - #wind2 *
			#lpf: LPF F 4000 I #hpf
			#out: ENV A 0.001 S 1 R 2.5 I #lpf #wind1 * .25 *
		`)).tosound();
		this.hooksnd=(new Audio.SFX(`
			#sig: NOISE
			#clk: SAW F #sig 1131 *
			#fil: BPF F 3000 B 2 I #clk #del - -1.6 > 1.6 <
			#del: DEL T 0.001 I #fil
			#out: ENV A 0.01 S 0 R 0.19 I #fil 0.15 *
		`)).tosound();
		this.normsnd=Audio.SFX.defsnd("kick");
		this.bodysnd=Audio.SFX.defsnd("thud",8,100,0.2);
		this.charsnd=Audio.SFX.defsnd("bell",0.05,313,5);
	}


	updateaudio() {
		for (let type of this.typearr) {
			let tdat=type.data;
			let vol=tdat.sndacc;
			let snd=tdat.snd;
			if (snd!==null && vol>0.001) {snd.play(vol);}
			tdat.sndacc=0;
		}
	}


	// ----------------------------------------
	// Graphics


	initgraphics() {
		// Setup the canvas
		this.canvas.style.imageRendering="pixelated";
		let draw=new Draw(this.canvas);
		this.draw=draw;
		// Particle paths.
		this.leafpath=new Draw.Path(`
			M-664-34-753-545-558-475-133-1000-69-857 269-966 201-648l161 69L94-334l26 46L712
			-402 591-197 974 0 591 197 712 402 120 288 94 334 362 579 201 648 269 966-69 857
			-133 1000-558 475-753 545-664 34-862 52V-52Z
		`,{dim:2,scale:0.001});
		let font=new Draw.Font();
		this.runearr=new Array(128);
		for (let i=0;i<128;i++) {
			let glyph=font.glyphs[i],path=null;
			if (glyph && i>32) {
				let w=glyph.width;
				path=new Draw.Path(glyph.path,{vec:[-1,-1/w],scale:2/w});
			}
			this.runearr[i]=path;
		}
		// Player path.
		let jupiter=new Draw.Path(`
			M102 236V-185c0-41 63-41 63 0V236H402c39 0 39 63 0 63H165V537c0 38-63 38-63 0V
			299H-403c-27 0-44-40-13-59 267-150 304-562-3-747-32-21-1-73 33-54C-75-373-64 14
			-301 236ZM-904-904H904V904H-904Zm64 64V840H840V-840Z
		`,{dim:2,scale:1/904});
		let back=new Draw.Path("M-.97-.97H.97V.97H-.97Z");
		this.playerpath=[
			[[128,128,255,255],back],
			[[64,64,128,255],jupiter]
		];
		// Init UI.
		let state=this;
		let ui=new UI(draw,this.input);
		this.ui=ui;
		ui.addpath(UI.VOLUME_PATH,{vec:[21,20],scale:15});
		let slider=ui.addslider(45,5,140,30,this.audio.volume);
		slider.onchange=function(node) {state.audio.setvolume(node.value);};
		this.cursor=new Draw.Path("M0 0 20 10H10V20Z");
		this.cursortrace=this.cursor.trace(0.5);
	}


	resize() {
		// Resize the canvas. Use absolute position to properly center on the screen.
		let canvas=this.canvas;
		let pad=[0,0],side=0;
		for (let elem of document.body.childNodes) {
			if (elem===canvas) {side=1;} else {pad[side]+=elem.clientHeight??0;}
		}
		let scale=this.scale,mindim=480;
		let wdif=this.worldmax.sub(this.worldmin);
		let maxw=Math.min(wdif[0]*scale,0x3fffffff)|0;
		let maxh=Math.min(wdif[1]*scale,0x3fffffff)|0;
		let winw=window.innerWidth,winh=window.innerHeight-pad[0]-pad[1];
		let winmax=mindim/(winw<winh?winw:winh);
		let draww=winw*winmax|0;
		let drawh=winh*winmax|0;
		draww=draww<maxw?draww:maxw;
		drawh=drawh<maxh?drawh:maxh;
		let ratio=winw/draww,ratioh=winh/drawh;
		ratio=ratio<ratioh?ratio:ratioh;
		maxw=draww*ratio;
		maxh=drawh*ratio;
		let style=canvas.style;
		style.left=(winw-maxw)*0.5+"px";
		style.top=pad[0]+"px";
		style.width=maxw+"px";
		style.height=maxh+"px";
		canvas.width=draww;
		canvas.height=drawh;
		this.resizebackground(draww,drawh);
	}


	resizebackground(draww,drawh) {
		let draw=this.draw;
		draw.img.resize(draww,drawh);
		this.drawrad=Math.sqrt(draww*draww+drawh*drawh)/this.scale;
		// Draw a dark forest.
		let bg=new Draw.Image(draww,drawh);
		this.background=bg;
		draw.pushstate();
		draw.setimage(bg);
		draw.fill(0,0,0,255);
		let rnd=new Random(9);
		let tower=new Draw.Path(`
			M-200 0V-617L-300-717V-867h76v75h55v-75h76v75h55v-75H-5v-133H5v14c19-5 98-19 60
			18 75 21 85 45 190 47-37 29-136 17-195 5-8-29-47-11-55-10v59H38v75H93v-75h76v75
			h55v-75h76v150L200-617V0Z
		`,{dim:2,scale:0.001});
		let tree=new Draw.Path(`
			M-43 0l22-110-89 35 16-35-93 35 46-72-143 47 36-26-102 14 32-22-100 5 185-108-93
			20 30-25-91 5 181-111-98 8 31-18-69 2 172-99-91 12 18-14-64 4 131-89-51 11 9-17
			-50 1 141-88-62 11 19-13-58-11 127-73-49 11 14-15-54-7 112-66-44 9 19-13-42-7 96
			-58-32 9 17-14-35 5 104-84 104 82-31-4 13 17-35-14 101 60-42 6 21 12-53-8 121 73
			-55 6 24 18-66-22 133 85-66 0 21 22-68-19 145 89-55 0 31 18-67-11 129 94-67-14
			38 28-102-22 168 104-65-7 30 20-103-16 180 116-79-2 13 24-85-25 181 108-88-11 31
			35-107-22 40 38-148-55 55 79-95-37 17 30-91-35 20 111Z
		`,{dim:2,scale:1/951});
		let fov=90*Math.PI/180;
		let f=drawh/(2*Math.tan(fov/2));
		let inv=Matrix.fromangles([-1.4*fov,0,0]).inv();
		let cam=new Vector([0,0,2]);
		let c=f*2/draww,den=c*inv[0]-inv[6];
		let xproj=new Vector([0,inv[7]-c*inv[1],inv[8]-c*inv[2]]).imul(1.1/den);
		let trees=~~(draww*drawh/239),toweri=~~(trees*0.9);
		for (let i=0;i<trees;i++) {
			let u=Math.pow(i/(trees-1),2);
			let p=(new Vector([0,(1-u)*11-1,0])).isub(cam);
			// Scale x to screen boundaries.
			p[0]=rnd.gets()*p.mul(xproj)-cam[0];
			let ip=inv.mul(p),x=ip[0],y=ip[1],z=ip[2],w=f/z;
			x=x*w+draww/2;
			y=y*w+drawh/2;
			if (i===toweri) {
				draw.setcolor(16,16,16,255);
				draw.fillpath(tower,{vec:[draww/2,y],scale:2.2*w});
			}
			let h=(rnd.getf()+0.5)*w;
			if (z<1e-7 || y-h>drawh) {continue;}
			// Shade based on proximity.
			let col=(rnd.getf()*6+27)/(1.5-u);
			draw.setcolor(col,1.33*col,col,255);
			draw.fillpath(tree,{vec:[x,y],scale:h});
		}
		draw.popstate();
	}


	// ----------------------------------------
	// Physics


	initphysics() {
		// Setup physics world.
		let world=new Phy.World(2);
		this.world=world;
		let state=this;
		world.maxsteptime=1/180;
		world.collcallback=function() {return state.collcallback(...arguments);};
		world.stepcallback=function(dt) {return state.stepcallback(dt);};
		world.deftype.release();
		let normmat=world.createbodytype(0.01,1.0   ,0.28,0.2);
		let wallmat=world.createbodytype(1.00,Infinity,0.95,0.5);
		let bodymat=world.createbodytype(0.01,1.0500,0.00,0.2);
		let ropemat=world.createbodytype(0.50,0.2763,0.00,0.0);
		let hookmat=world.createbodytype(0.25,0.5525,0.25,0.0);
		let edgemat=world.createbodytype(0.01,0.5525,0.50,0.0);
		let partmat=world.createbodytype(0.50,1e-9  ,0.50,0.0);
		let leafmat=world.createbodytype(0.75,1e-8  ,0.00,1.0);
		let rainmat=world.createbodytype(0.25,1e-8  ,0.00,0.0);
		let charmat=world.createbodytype(0.75,1e-8  ,0.25,0.0);
		world.deftype=normmat;
		this.typearr=[];
		for (let type of world.typelist.iter()) {
			this.typearr.push(type);
		}
		partmat.gravity=new Vector([0,0]);
		charmat.gravity=new Vector([0,0]);
		// Attach metadata to materials.
		function addmeta(type,rgb,path,vert,snd=null,sndmin=0,sndmax=0,freq=0,bscale=1,dscale=1,ang=0,life=0) {
			let t=type.data;
			t.rgb=rgb;
			t.drawpath=path;
			t.bodyvert=vert;
			t.snd=snd;
			t.sndacc=0;
			t.sndmin=sndmin;
			t.sndmax=sndmax;
			t.partfreq=freq;
			t.bodyscale=bscale;
			t.drawscale=dscale;
			t.partang=ang;
			t.partlife=life;
		}
		let rw=1/.553,PI2=Math.PI/2;
		let leafvert=[[-0.753,-0.545],[-0.133,-1],[0.269,-0.966],[0.973,0],[0.269,0.966],[-0.133,1],[-0.753,0.545]];
		let leafpath=this.leafpath;
		let rainvert=[[1,0],[0,1],[-8,0],[0,-1]];
		let runevert=[[-1,-rw],[1,-rw],[1,rw],[-1,rw]];
		let partvert=[[-1,-1],[1,-1],[1,1],[-1,1]];
		addmeta(normmat,[255,255,255,255],null,null,this.normsnd,20,40);
		addmeta(wallmat,[140,170,140,255],null,null);
		addmeta(bodymat,[128,128,255,255],null,null,this.bodysnd,10,30);
		addmeta(ropemat,null             ,null,null);
		addmeta(hookmat,null             ,null,null,this.hooksnd, 2, 6);
		addmeta(edgemat,[100,100,100,255],null,null,this.hooksnd, 2, 6);
		addmeta(partmat,[255,255,255,255],null    ,partvert,null        , 0, 0,NaN ,0.05,25  ,NaN,0.2);
		addmeta(leafmat,[255,255,255,255],leafpath,leafvert,this.leafsnd, 6,10,1000,0.3 ,2   ,NaN,40 );
		addmeta(rainmat,[128,128,255,255],null    ,rainvert,this.rainsnd,10,15,200 ,0.1 ,10  ,PI2,9  );
		addmeta(charmat,[255,255,255,255],null    ,runevert,this.charsnd,.1, 1,NaN ,1   ,1   ,0  ,10 );
	}


	static bodyinit(body) {
		// Setup body metadata.
		let data=body.data;
		if (data.scale!==undefined) {return data;}
		data.scale=1;
		data.pos=null;
		data.accel=0;
		data.sndacc=0;
		data.sndready=true;
		data.time=0;
		data.life=0;
		data.paths=[];
		let type=body.type,tdat=type.data;
		let rgb=tdat.rgb;
		if (rgb!==null) {
			let path=tdat.drawpath;
			if (path===null) {
				path=new Draw.Path();
				for (let v of body.vertarr) {path.lineto(v);}
				path.close();
			}
			if (type.id===WALL) {
				data.paths=[[[32,32,32,255],path]];
				path=path.trace(0,-0.2);
			}
			data.paths.push([rgb.slice(),path]);
		}
		return data;
	}


	collcallback(intr,a,acon,b,bcon,norm,veldif,posdif) {
		let aid=a.type.id,bid=b.type.id;
		if (aid===WALL && bid===WALL) {
			return false;
		} else if (aid>=PART || bid>=PART) {
			if ((aid>=CHAR && (bid<BODY || bid>EDGE))
			||  (bid>=CHAR && (aid<BODY || aid>EDGE))) {return false;}
		} else if ((aid>=ROPE && aid<=EDGE) || (bid>=ROPE && bid<=EDGE)) {
			if (aid>=BODY && aid<=EDGE && bid>=BODY && bid<=EDGE) {return false;}
			if (this.throwing<2) {return false;}
			// Allow the hook to form bonds.
			let uid=aid,vid=bid;
			if (vid===EDGE) {uid=bid;vid=aid;}
			if (this.throwing===3 && uid===EDGE && (vid===NORM || vid===WALL)) {
				// Check for other bonds.
				let al=a.bondlist,bl=b.bondlist;
				let link=(al.count<bl.count?al:bl).head;
				while (link!==null) {
					let bond=link.obj;
					let u=bond.a,v=bond.b;
					if ((u===a && v===b) || (u===b && v===a)) {break;}
					link=link.next;
				}
				if (link===null) {
					let world=a.world;
					let ainv=aid===EDGE?new Vector(2):a.mat.inv().mul(acon);
					let binv=bid===EDGE?new Vector(2):b.mat.inv().mul(bcon);
					let bond=world.createbond(a,ainv,b,binv,0,6000);
					bond.breakdist=1.5;
				}
				return false;
			}
		}
		// Track each body's acceleration.
		let u=a.mass/(a.mass+b.mass);
		u=u<1?u:1;
		let adat=a.data,bdat=b.data;
		adat.accel+=veldif*(1-u);
		bdat.accel+=veldif*u;
		// Scale sound by distance.
		let apos=a.pos,bpos=b.pos;
		let cenx=(acon[0]+apos[0]+bcon[0]+bpos[0])*0.5;
		let ceny=(acon[1]+apos[1]+bcon[1]+bpos[1])*0.5;
		let cam=this.camcen;
		let dx=cam[0]-cenx,dy=cam[1]-ceny;
		let vol=1-(dx*dx+dy*dy)/this.snddist2;
		if (vol>0) {
			adat.sndacc+=veldif*vol*(1-u);
			bdat.sndacc+=veldif*vol*u;
		}
		// Create a particle between the 2 bodies.
		if (veldif>2.0 && aid<PART && bid<PART) {
			let mag=posdif*0.5*this.rnd.getf();
			let dir=Vector.random(2);
			let cen=[cenx+dir[0]*mag,ceny+dir[1]*mag];
			let avel=a.relvel(acon),bvel=b.relvel(bcon);
			mag=veldif*this.rnd.getf();
			dir[0]=dir[0]*mag+avel[0]+bvel[0];
			dir[1]=dir[1]*mag+avel[1]+bvel[1];
			this.createparticle(PART,cen,dir);
		}
		return true;
	}


	stepcallback(dt) {
		// Countdown the charge timer.
		if (this.throwing===2) {
			let rate=this.chargerate;
			this.charge-=dt*rate;
			if (this.charge<=0) {
				dt=-this.charge/rate;
				this.charge=0;
				this.throwing=3;
			}
		}
		if (this.throwing===3) {
			// If we've thrown for long enough, begin pulling.
			let pulltime=this.pulltime;
			let hooktime=this.hooktime;
			if (hooktime<pulltime) {
				hooktime+=dt;
				hooktime=hooktime<pulltime?hooktime:pulltime;
				this.hooktime=hooktime;
				// Expand the hook.
				let distmul=hooktime*4;
				distmul=(distmul<1?distmul:1)*4+1;
				for (let bond of this.hookbond) {
					bond.dist=bond.data.orig*distmul;
				}
				// Increase rope tension.
				let u=hooktime/pulltime;
				let tension=this.ropemin+u*u*(this.ropemax-this.ropemin);
				for (let body of this.ropebody) {
					for (let bond of body.bonditer()) {
						if (bond.breakdist===Infinity) {
							bond.tension=tension;
						}
					}
				}
			}
			// Color code the rope based on hook tension.
			let h=this.ropeheat;
			for (let body of this.edgebody) {
				for (let bond of body.bonditer()) {
					let max=bond.breakdist;
					if (max<Infinity) {
						let u=bond.relapos().dist(bond.relbpos())/max;
						h=h>u?h:u;
					}
				}
			}
			this.ropeheat=h;
		}
	}


	// ----------------------------------------
	// Particles


	createbiome(id,pos,dim) {
		let type=this.typearr[id],tdat=type.data;
		let freq=tdat.partfreq/(dim[0]*dim[1]);
		// Seed the biome particles.
		let time=(id===RAIN?0.5:1)*tdat.partlife;
		let biome={type:id,time:time,freq:freq,pos:new Vector(pos),dim:new Vector(dim)};
		this.biomearr.push(biome);
		return biome;
	}


	createtext(text,pos,scale) {
		pos=new Vector(pos);
		let off=new Vector(2);
		for (let i=0;i<text.length;i++) {
			let c=text.charCodeAt(i);
			if (c>32 && c<128) {this.createparticle(CHAR+c,pos.add(off),{scale:scale});}
			else if (c===10) {off[0]=0;off[1]+=scale*2.5;}
			off[0]+=scale*2;
		}
	}


	createparticle(id,pos,opt={}) {
		// opt: ang, time, rad, rgb, rand, vel
		let type=this.typearr[id<CHAR?id:CHAR],tdat=type.data;
		let life=tdat.partlife*(id<CHAR?this.rnd.getf()*0.5+1:1);
		// If the particle is too far away, don't spawn it.
		if (id>=PART && id!==CHAR) {
			let dist=this.playerbody.pos.dist(pos);
			let drawdist=this.drawrad+life*this.maxspeed;
			if (dist>drawdist) {return null;}
		}
		let ang=opt.ang??tdat.partang;
		ang=isNaN(ang)?this.rnd.getf()*Math.PI*2:ang;
		let vert=[],scale=opt.scale??tdat.bodyscale;
		for (let v of tdat.bodyvert) {vert.push([v[0]*scale,v[1]*scale]);}
		if (id>=CHAR) {tdat.drawpath=this.runearr[id-CHAR];}
		let body=this.world.createbody(vert,pos,[ang],type);
		let data=Game.bodyinit(body);
		body.vel.set(opt.vel??0);
		data.scale=scale;
		if (opt.rand) {body.vel.iadd(Vector.random(2).imul(opt.rand));}
		data.life=life;
		data.time=opt.time??0;
		let orgb=opt.rgb;
		if (orgb) {
			let rgb=data.paths[0][0];
			for (let i=0;i<4;i++) {rgb[i]=orgb[i];}
		}
		return body;
	}


	updatebiomes(dt) {
		// Environmental particles.
		let rnd=this.rnd;
		for (let biome of this.biomearr) {
			let pos=biome.pos,dim=biome.dim,id=biome.type;
			let opt={};
			if (id===RAIN) {opt.vel=this.world.gravity.mul(3);}
			let time=biome.time+dt,freq=biome.freq;
			while (time>=freq) {
				time-=freq;
				let x=rnd.getf()*dim[0]+pos[0];
				let y=rnd.getf()*dim[1]+pos[1];
				if (id===LEAF) {
					let u=rnd.getf();
					opt.rgb=[128+84*u,70+105*u,27+28*u,255];
				}
				this.createparticle(id,[x,y],opt);
			}
			biome.time=time;
		}
	}


	// ----------------------------------------
	// Player


	initplayer(playerpos) {
		// Setup the player.
		let world=this.world;
		this.throwing=-1;
		let typearr=this.typearr;
		this.camcen.set(playerpos);
		this.playerbody=world.createbody([[-1,-1],[1,-1],[1,1],[-1,1]],playerpos,null,typearr[BODY]);
		Game.bodyinit(this.playerbody).paths=this.playerpath;
		// Create the hook.
		let hookspace=0.135;
		this.hookang=0;
		this.hooktime=0;
		this.hookbond=[];
		this.edgebody=[];
		let hookbody=[];
		let len=3;
		for (let s=0;s<3;s++) {
			let ang=(s-1)*1.3;
			let dx=Math.cos(ang)*hookspace,dy=Math.sin(ang)*hookspace;
			for (let i=s?1:0;i<=len;i++) {
				let pos=new Vector([len*hookspace-dx*i,dy*i]);
				let mat=typearr[i===len?EDGE:HOOK];
				let body=world.createsphere(0.16,8,pos.add(playerpos),0,mat);
				let data=Game.bodyinit(body);
				data.pos=pos;
				let disp=null;
				if (i===len) {
					this.edgebody.push(body);
					disp=data.paths[0][0];
				}
				let bsup=(i%len)?0:1;
				for (let j=0;j<hookbody.length;j++) {
					let n=hookbody[j],nsup=(j%len)?0:1;
					let bonds=[0,1,10][nsup+bsup];
					for (let b=0;b<bonds;b++) {
						let bond=world.createbond(n,[0,0],body,[0,0],-1,6000);
						this.hookbond.push(bond);
						bond.data.orig=bond.dist;
						bond.data.rgb=disp;
						disp=null;
					}
				}
				hookbody.push(body);
			}
		}
		this.hookbody=hookbody;
		// Create the rope.
		let segs=20,bonds=8;
		let ropespace=0.7/segs;
		this.pulltime=3;
		this.ropemin=880/bonds;
		this.ropemax=88000/bonds;
		this.ropebody=[];
		this.ropeheat=0;
		this.ropergb=[255,255,255,255];
		let prev=this.playerbody;
		for (let i=segs;i;i--) {
			let body=hookbody[len*2];
			if (i>1) {
				body=world.createsphere(0.16,8,playerpos,null,typearr[ROPE]);
				this.ropebody.push(body);
			}
			for (let b=0;b<bonds;b++) {
				let bond=world.createbond(prev,[0,0],body,[0,0],ropespace,this.ropemax);
				if (!b) {bond.data.rgb=this.ropergb;}
			}
			prev=body;
		}
		this.ropehook=this.ropebody.concat(hookbody);
	}


	updateplayer(dt) {
		let playerbody=this.playerbody;
		if (!playerbody) {return;}
		let playerpos=playerbody.pos;
		let playervel=playerbody.vel;
		// Center the camera on the player. Use a leash distance to prevent the camera
		// from jittering if the player is rocking.
		let camcen=this.camcen;
		let leash=0.5/camcen.dist(playerpos),scale=this.scale;
		leash=leash<1?leash:1;
		let camera=this.camera;
		let img=this.draw.img;
		let drawmax=[img.width,img.height];
		for (let i=0;i<2;i++) {
			let pos=camcen[i]*leash+playerpos[i]*(1-leash);
			camcen[i]=pos;
			let ds=drawmax[i]/scale;
			pos-=ds*0.5;
			let min=this.worldmin[i];
			let max=this.worldmax[i]-ds;
			pos=pos>min?pos:min;
			pos=pos<max?pos:max;
			camera.vec[i]=-pos*scale;
		}
		// Play wind rushing sound.
		let windnew=playervel.mag();
		let wind=this.windvol;
		wind=wind>windnew?wind:windnew;
		this.windvol=wind*Math.pow(0.2,dt);
		let snd=this.windinst;
		wind=wind/40-0.1;
		if (wind>0) {
			wind=wind<1?wind:1;
			if (snd===null) {snd=this.windsnd.play(wind);}
			snd.setvolume(wind);
		} else if (snd) {
			snd.remove();
			snd=null;
		}
		this.windinst=snd;
		// Check for click.
		let input=this.input;
		let throwing=this.throwing;
		let charge=this.charge;
		let key=input.MOUSE.LEFT;
		let click=input.getkeychange(key);
		input.keyclear(key);
		// Throw if we can, or release.
		if (throwing<=0) {throwing=click>0?1:0;}
		if (throwing===1) {charge+=dt*this.chargerate;}
		charge=charge<1?charge:1;
		if (click<0) {throwing=throwing===1?2:0;}
		if ((this.throwing!==throwing && throwing<3) || throwing===1) {
			// Handle swinging, throwing, and resting.
			charge=throwing>0?charge:0;
			let rad=charge*1.1,spin=25;
			let ang=(this.hookang+dt*spin)%(Math.PI*2);
			this.hookang=ang;
			let look=ang-Math.PI*0.8*charge;
			let dirx=Math.cos(look),diry=Math.sin(look);
			snd=this.spininst;
			if (throwing!==1) {
				// Throw in mouse direction.
				if (snd!==null) {snd.remove();snd=null;}
				rad=0;
				let dif=camera.inv().apply(this.mouse).isub(playerpos).normalize();
				dirx=dif[0];diry=dif[1];
			} else if (snd===null) {
				snd=this.spinsnd.play(1,0,dt);
			}
			this.spininst=snd;
			this.hooktime=0;
			// Calculate rope and hook positions and velocities.
			let ropehook=this.ropehook;
			let tension=throwing<2?this.ropemax:this.ropemin;
			let rope=this.ropebody.length,len=ropehook.length;
			let old=[0,0];
			for (let i=0;i<len;i++) {
				let body=ropehook[i];
				let u=i/rope;
				if (i>=rope) {old=body.data.pos;u=1;}
				let pos=body.pos,vel=body.vel;
				let v=u*charge,r=u*rad,a=ang-1.5*v;
				let nx=old[0]*dirx-old[1]*diry+Math.cos(a)*r;
				let ny=old[0]*diry+old[1]*dirx+Math.sin(a)*r;
				let dx=-ny,dy=nx;
				// Throw the hook.
				if (throwing!==1) {
					spin=35*v;
					dx=dirx;
					dy=diry;
				}
				pos[0]=playerpos[0]+nx;
				pos[1]=playerpos[1]+ny;
				vel[0]=playervel[0]+dx*spin;
				vel[1]=playervel[1]+dy*spin;
				// Delete temporary bonds. Reset rope tension and hook size.
				for (let bond of body.bonditer()) {
					let orig=bond.data.orig;
					if (!(bond.breakdist===Infinity)) {bond.release();}
					else if (orig) {bond.dist=orig;}
					else {bond.tension=tension;}
				}
			}
		}
		this.throwing=throwing;
		this.charge=charge;
		// Glow according to hook tension.
		let u=this.ropeheat;
		this.ropeheat=u*Math.pow(0.3,dt);
		let rgb=this.ropergb;
		u=u<1?u:1;
		rgb[1]=255-200*u;
		rgb[2]=255-250*u;
	}


	// ----------------------------------------
	// Main


	initlevel() {
		// Set up the level.
		this.reset();
		let world=this.world;
		let toph=15,both=15,climb=200;
		let miny=-toph-climb,maxy=both;
		let minx=0,maxx=200;
		this.worldmin=new Vector([minx,miny]);
		this.worldmax=new Vector([maxx,maxy]);
		this.initplayer([maxx*0.5-24,10]);
		// Create walls
		let typearr=this.typearr;
		let wallmat=typearr[WALL];
		let wallt=4,over=3.6;
		let wallw=(maxx-minx+wallt)*0.5,wallx=(minx+maxx)*0.5;
		let wallh=(maxy-miny+wallt)*0.5,wally=(miny+maxy)*0.5;
		let topvert =[[-wallw,-wallt],[wallw,-wallt],[wallw,wallt],[-wallw,wallt]];
		let sidevert=[[-wallt,-wallh],[wallt,-wallh],[wallt,wallh],[-wallt,wallh]];
		world.createbody(topvert ,[wallx,miny-over],null,wallmat);
		world.createbody(topvert ,[wallx,maxy+over],null,wallmat);
		world.createbody(sidevert,[minx-over,wally],null,wallmat);
		world.createbody(sidevert,[maxx+over,wally],null,wallmat);
		// Create top and bottom barriers.
		let bart=1.5,barw=wallw/5;
		let verts=[[-barw,-bart],[barw,-bart],[barw,bart],[-barw,bart]];
		for (let i=0;i<3;i++) {
			let x=(i*4+1)*barw;
			world.createbody(verts,[x,-bart],null,wallmat);
			world.createbody(verts,[x,miny+toph],null,wallmat);
		}
		verts=[[-3,-3],[3,-3],[3,3],[-3,3]];
		for (let i=1;i<3;i++) {
			world.createbody(verts,[maxx*i/3,both*0.25],[Math.PI/4],wallmat);
		}
		// Create holds
		let rnd=this.rnd;
		let holdgap=20;
		let levels=Math.round(climb/holdgap)|1;
		holdgap=climb/levels;
		let spanx=2*Math.floor((maxx-minx)/(2*holdgap));
		let gapx=(maxx-minx)/spanx;
		for (let l=0;l<levels;l++) {
			let holdy=maxy-both-l*holdgap;
			for (let s=1-(l&1);s<=spanx;s+=2) {
				verts=[];
				for (let i=0;i<8;i++) {
					verts.push([rnd.gets()*8,rnd.gets()*8]);
				}
				let holdx=s*gapx+minx;
				world.createbody(verts,[holdx,holdy],null,wallmat);
			}
		}
		// Extra objects.
		world.createsphere(3,5,[100,-9],null,typearr[NORM]);
		this.createtext("CLICK TO THROW",[maxx*0.5-33,-4],1);
		this.createbiome(LEAF,[minx,0],[maxx,both]);
		this.createbiome(RAIN,[minx,-climb+bart],[maxx,climb-bart*2]);
		// Game state
		this.climbtime=0;
		this.climbstop=-climb;
		this.climb=0;
		for (let body of this.world.bodyiter()) {
			Game.bodyinit(body);
		}
	}


	updatebodies(dt) {
		let draw=this.draw;
		let decay=Math.pow(0.01,dt);
		let trans=this.camera;
		for (let body of this.world.bodyiter()) {
			let bdat=body.data;
			let tdat=body.type.data,id=body.type.id;
			let pos=body.pos;
			// Allow the accumulated acceleration to gradually decay.
			let accel=bdat.accel;
			bdat.accel=accel*decay;
			let sndmin=tdat.sndmin;
			if (accel<sndmin*0.25) {
				bdat.sndready=true;
			} else if (accel>=sndmin && bdat.sndready) {
				// Play a sound.
				bdat.sndready=false;
				let vol=bdat.sndacc/tdat.sndmax;
				tdat.sndacc+=vol<1?vol:1;
			}
			bdat.sndacc*=decay;
			// Update particle state.
			let paths=bdat.paths;
			if (id>=PART) {
				let time=bdat.time+dt;
				let life=bdat.life;
				let rgb=paths[0][0];
				let vel=body.vel;
				// Destroy rain or snow if they're hit hard.
				if (id===RAIN && accel>0.1) {
					if (time>0.05) {
						for (let j=0;j<8;j++) {
							this.createparticle(PART,pos,{vel:vel,rand:2,rgb:rgb});
						}
					}
					time=life;
				}
				if (!(time>=0 && time<life)) {
					body.release();
					continue;
				}
				if (id>=CHAR) {
					// Start timer when character is hit.
					let lim=life-1;
					if (accel>0.01 && time<lim) {time=lim;}
					time=time>=lim?time:2;
				}
				bdat.time=time;
				let half=time>life-time?life-time:time;
				let fade=(life>2 || time>half)?Math.max(life*0.05,0.1):0;
				rgb[3]=255.99*(half<fade?half/fade:1)|0;
			}
			// Draw
			let bodytrans=trans.apply({mat:body.mat,vec:pos});
			bodytrans.scalemat(bdat.scale*tdat.drawscale);
			for (let p of paths) {
				draw.setcolor(p[0]);
				draw.fillpath(p[1],bodytrans);
			}
		}
	}


	update(frametime) {
		// Get the timestep. Prevent steps that are too large.
		let dt=(frametime-this.frameprev)*0.001;
		dt=dt<this.framemax?dt:this.framemax;
		dt=dt>0?dt:0;
		this.frameprev=frametime;
		let hide=!Env.IsVisible(this.canvas);
		this.audio.mute(hide);
		this.audio.update();
		if (hide) {return true;}
		let starttime=performance.now();
		let input=this.input;
		input.update();
		let draw=this.draw;
		let mouse=this.mouse;
		mouse.set(input.getmousepos());
		this.ui.update();
		draw.img.data32.set(this.background.data32);
		let world=this.world;
		world.update(dt);
		this.updateplayer(dt);
		this.updatebiomes(dt);
		this.updatebodies(dt);
		// Draw the charge timer.
		draw.pushstate();
		draw.settransform(this.camera);
		let charge=this.charge;
		if (charge>0) {
			let pos=this.playerbody.pos,x=pos[0],y=pos[1];
			let ang0=-Math.PI*0.5,ang1=ang0+Math.PI*2*charge;
			let path=draw.begin();
			path.addarc(x,y,ang0,ang1,1.75,1.75,true);
			path.addarc(x,y,ang1,ang0,1.25,1.25,true);
			path.close();
			draw.setcolor(100,100,200,255);
			draw.fillpath();
		}
		// Draw the bonds.
		draw.linewidth=0.15;
		for (let bond of world.bonditer()) {
			let rgb=bond.data.rgb;
			if (!rgb) {continue;}
			draw.setcolor(rgb);
			let apos=bond.relapos(),bpos=bond.relbpos();
			draw.drawline(apos[0],apos[1],bpos[0],bpos[1]);
		}
		draw.popstate();
		// Update objective.
		let stop=this.climbstop;
		let climb=Math.min(this.climb,this.playerbody.pos[1]);
		let rem=climb-stop;
		if (rem<=0) {climb=stop;rem=0;}
		else if (climb<0) {this.climbtime+=dt;}
		this.climb=climb;
		// Draw the HUD.
		this.updateaudio();
		this.ui.render();
		draw.setcolor(255,255,255,255);
		draw.filltext(draw.img.width-82,5,this.framestr,20);
		draw.filltext(5,45,`finish: ${Math.ceil(rem)} m`,20);
		if (rem<=0 || climb>=0) {draw.setcolor(128,128,255,255);}
		draw.filltext(5,70,`time  : ${this.climbtime.toFixed(2)} s`,20);
		draw.setcolor(128,128,255,255);
		draw.fillpath(this.cursor,{vec:mouse});
		draw.setcolor(255,255,255,255);
		draw.fillpath(this.cursortrace,{vec:mouse});
		draw.screenflip();
		// Calculate the frame time.
		this.framesum+=performance.now()-starttime;
		if (++this.frames>=100) {
			let avg=this.framesum/this.frames;
			this.framestr=avg.toFixed(1)+" ms";
			this.frames=0;
			this.framesum=0;
		}
		return true;
	}

}
