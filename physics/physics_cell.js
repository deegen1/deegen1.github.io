...
class PhyBroadphase {
	...
	constructor(world) {
		...
		this.cell64=new BigUint64Array(0);
	}
	...
	resize(size) {
		if (size===this.cell64.length) {
			return;
		}
		var data=this.celldata;
		if (size>data.length*8) {
			var nsize=size;
			for (var i=1;i<32;i+=i) {nsize|=nsize>>1;}
			data=new Uint8Array((nsize+1)*8);
			var endian=(new Uint32Array((new BigUint64Array([4n])).buffer))[0];
			this.celldata=data;
			this.cellhi=new Uint32Array(data.buffer,  endian);
			this.celllo=new Uint32Array(data.buffer,4-endian);
		}
		this.cell64=new BigUint64Array(data.buffer,0,size);
	}
...
class PhyScene {
	...
	update() {
		...
		ctx.fillText("FPS: "+this.fpsstr,5,20);
